data:extend({
  {
    type = "item",
    name = "pm-mixed-bastnasite",
    icon_size = 64,
    icon = "__periodic-madness__/graphics/icons/ores/mixed-bastnasite.png",
    pictures =
    {
      { filename = "__periodic-madness__/graphics/icons/ores/mixed-bastnasite.png", size = 64, scale = 0.5},
      { filename = "__periodic-madness__/graphics/icons/ores/mixed-bastnasite-2.png", size = 64, scale = 0.45},
      { filename = "__periodic-madness__/graphics/icons/ores/mixed-bastnasite-3.png", size = 64, scale = 0.5},
      { filename = "__periodic-madness__/graphics/icons/ores/mixed-bastnasite-4.png", size = 64, scale = 0.5},
      { filename = "__periodic-madness__/graphics/icons/ores/mixed-bastnasite-5.png", size = 64, scale = 0.5},
      { filename = "__periodic-madness__/graphics/icons/ores/mixed-bastnasite-6.png", size = 64, scale = 0.43},
    },
    subgroup = "pm-yttrium-tm",
    order = "a",
    stack_size = 300
  },
  {
    type = "item",
    name = "pm-bastnasite-Y",
    icon_size = 64,
    icon = "__periodic-madness__/graphics/icons/ores/bastnasite-Y.png",
    pictures =
    {
      { filename = "__periodic-madness__/graphics/icons/ores/bastnasite-Y.png", size = 64, scale = 0.5},
      { filename = "__periodic-madness__/graphics/icons/ores/bastnasite-Y-2.png", size = 64, scale = 0.5},
      { filename = "__periodic-madness__/graphics/icons/ores/bastnasite-Y-3.png", size = 64, scale = 0.5},
      { filename = "__periodic-madness__/graphics/icons/ores/bastnasite-Y-4.png", size = 64, scale = 0.5},
    },
    subgroup = "pm-yttrium-tm",
    order = "b",
    stack_size = 200
  },
  {
    type = "item",
    name = "pm-bastnasite-C",
    icon_size = 64,
    icon = "__periodic-madness__/graphics/icons/ores/bastnasite-C.png",
    pictures =
    {
      { filename = "__periodic-madness__/graphics/icons/ores/bastnasite-C.png", size = 64, scale = 0.5},
      { filename = "__periodic-madness__/graphics/icons/ores/bastnasite-C-2.png", size = 64, scale = 0.5},
      { filename = "__periodic-madness__/graphics/icons/ores/bastnasite-C-3.png", size = 64, scale = 0.5},
      { filename = "__periodic-madness__/graphics/icons/ores/bastnasite-C-4.png", size = 64, scale = 0.5},
    },
    subgroup = "pm-yttrium-tm",
    order = "c",
    stack_size = 200
  },
  {
    type = "item",
    name = "pm-yttrium-oxide",
    icon_size = 64,
    icon = "__periodic-madness__/graphics/icons/ores/yttrium-oxide.png",
    pictures =
    {
      { filename = "__periodic-madness__/graphics/icons/ores/yttrium-oxide.png", size = 64, scale = 0.5},
      { filename = "__periodic-madness__/graphics/icons/ores/yttrium-oxide-2.png", size = 64, scale = 0.5},
      { filename = "__periodic-madness__/graphics/icons/ores/yttrium-oxide-3.png", size = 64, scale = 0.5},
    },
    subgroup = "pm-yttrium-tm",
    order = "d",
    stack_size = 200
  },
  {
    type = "item",
    name = "pm-yttrium-ore",
    icon_size = 64,
    icon = "__periodic-madness__/graphics/icons/ores/yttrium-ore-icon.png",
    pictures =
    {
      {filename = "__periodic-madness__/graphics/icons/ores/yttrium-ore-icon.png", size = 64, scale = 0.5},
      {filename = "__periodic-madness__/graphics/icons/ores/yttrium-ore-icon-2.png", size = 64, scale = 0.5},
      {filename = "__periodic-madness__/graphics/icons/ores/yttrium-ore-icon-3.png", size = 64, scale = 0.5},
    },
    subgroup = "pm-yttrium-tm",
    order = "g",
    protons = 39,
    neutrons = 50,
    stack_size = 200
  },

  {
    type = "recipe",
    name = "pm-mixed-bastnasite",
    enabled = false,
    energy_required = 12,
    categories = {"pm-acids"},
    subgroup = "pm-yttrium-tm",
    order = "a",
    allow_productivity = true,
    ingredients =
    {
      PM.ingredient("pm-transition-metals-ore", 16),
      PM.ingredient("pm-chromic-acid", 20, "fluid")
    },
    results =
    {
      PM.product("pm-mixed-bastnasite"):amount(3, 8):done()
    }
  } --[[@as data.RecipePrototype]],
  {
    type = "recipe",
    name = "pm-mixed-bastnasite-crushing",
    icon_size = 128,
    icon = "__periodic-madness__/graphics/icons/recipes/bastnasite-seperation.png",
    enabled = false,
    energy_required = 6,
    categories = {"pm-crushing"},
    subgroup = "pm-yttrium-tm",
    order = "b",
    allow_productivity = true,
    ingredients =
    {
      PM.ingredient("pm-mixed-bastnasite", 6),
      PM.ingredient("pm-ion-exchange-beads", 9)
    },
    results =
    {
      PM.product("pm-bastnasite-Y"):amount(2, 4):chance(0.5):done(),
      PM.product("pm-bastnasite-C"):amount(2, 4):chance(0.5):done()
    }
  } --[[@as data.RecipePrototype]],
  {
    type = "recipe",
    name = "pm-bastnasite-Y-seperation",
    icon_size = 128,
    icon = "__periodic-madness__/graphics/icons/recipes/bastnasite-Y-seperation.png",
    enabled = false,
    energy_required = 16,
    categories = {"chemistry"},
    subgroup = "pm-yttrium-tm",
    order = "c",
    main_product = "pm-yttrium-oxide",
    ingredients =
    {
      PM.ingredient("pm-bastnasite-Y", 6),
      PM.ingredient("sulfuric-acid", 15, "fluid"),
      PM.ingredient("pm-vanadium-oxide-catalyst", 4)
    },
    results =
    {
      PM.product("pm-yttrium-oxide"):amount(1, 3):done(),
      PM.product("pm-vanadium-oxide-catalyst"):amount(4):chance(0.15, 1):catalyst(4):done(),
      PM.product("pm-catalyst-container"):amount(4):chance(0, 0.15):catalyst(4):done(),
      PM.product("pm-flourine"):amount(5):chance(0.456):done()
    }
  } --[[@as data.RecipePrototype]],
  {
    type = "recipe",
    name = "pm-bastnasite-C-seperation",
    icon_size = 128,
    icon = "__periodic-madness__/graphics/icons/recipes/bastnasite-C-seperation.png",
    enabled = false,
    energy_required = 16,
    categories = {"chemistry"},
    subgroup = "pm-yttrium-tm",
    order = "d",
    main_product = "pm-yttrium-oxide",
    ingredients =
    {
      PM.ingredient("pm-bastnasite-C", 6),
      PM.ingredient("sulfuric-acid", 15, "fluid"),
      PM.ingredient("pm-cobalt-catalyst", 4)
    },
    results =
    {
      PM.product("pm-yttrium-oxide"):amount(0, 1):done(),
      PM.product("pm-cobalt-catalyst"):amount(4):chance(0.25, 1):catalyst(4):done(),
      PM.product("pm-catalyst-container"):amount(4):chance(0, 0.25):catalyst(4):done(),
      PM.product("pm-carbon-trioxide", "fluid"):amount(8, 10):done()
    }
  } --[[@as data.RecipePrototype]],
  {
    type = "recipe",
    name = "pm-yttrium-fluoride",
    enabled = false,
    energy_required = 12,
    categories = {"chemistry"},
    subgroup = "pm-yttrium-tm",
    order = "f",
    main_product = "pm-aqueous-yttrium-fluoride",
    ingredients =
    {
      PM.ingredient("pm-yttrium-oxide", 2),
      PM.ingredient("pm-hydrogen-flouride", 8, "fluid")
    },
    results =
    {
      PM.product("pm-aqueous-yttrium-fluoride", "fluid"):amount(5):done(),
      PM.product("water", "fluid"):amount(10):done()
    }
  } --[[@as data.RecipePrototype]],
  {
    type = "recipe",
    name = "pm-yttrium-ore",
    enabled = false,
    energy_required = 8,
    categories = {"pm-washing"},
    subgroup = "pm-yttrium-tm",
    order = "g",
    crafting_machine_tint = 
    {
        primary = {0.8, 0.937, 0.961}
    },
    ingredients =
    {
      PM.ingredient("pm-aqueous-yttrium-fluoride", 5, "fluid"),
      PM.ingredient("pm-ammonium-chloride", 2)
    },
    results =
    {
      PM.product("pm-yttrium-ore"):amount(5):done()
    }
  } --[[@as data.RecipePrototype]],

  {
    type = "recipe",
    name = "pm-pure-bastnasite-crushing",
    icon_size = 64,
    icon = "__periodic-madness__/graphics/icons/recipes/pure-bastnasite-separation.png",
    enabled = false,
    energy_required = 6,
    categories = {"pm-crushing"},
    subgroup = "pm-yttrium-tm",
    order = "ba",
    allow_productivity = true,
    ingredients =
    {
      PM.ingredient("pm-pure-bastnasite", 8),
      PM.ingredient("pm-ion-exchange-beads", 6)
    },
    results =
    {
      PM.product("pm-bastnasite-Y"):amount(3, 6):done(),
      PM.product("pm-bastnasite-C"):amount(3, 6):done(),
      PM.product("pm-bastnasite-ce"):amount(0, 3):chance(0.75):done()
    }
  } --[[@as data.RecipePrototype]],
})
